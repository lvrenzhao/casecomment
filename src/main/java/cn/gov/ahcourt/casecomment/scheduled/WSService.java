...
//    }
    ...
    public WsAjid getWsAjid(String ajid){
        return wsAjidMapper.selectByPrimaryKey(ajid);
    }

    public int saveWsAjid(WsAjid bean){
        try {
            if (bean != null && bean.getUpdatetime() != null) {
                return wsAjidMapper.updateByPrimaryKey(bean);
            } else {
                return wsAjidMapper.insert(bean);
            }
        }catch (Exception ex){
            ex.printStackTrace();
        }
    ...
    public int deleteAjidByFjm(String fjm){
        try{
            wsAjidMapper.deleteByFjm(fjm);
        }catch (Exception ex){
            ex.printStackTrace();
        }
        return -1;
